...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ numero: string; id: string }> }) {
  try {
    ...
    const itemExistente = existingRows[0]
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ numero: string; id: string }> }) {
  try {
    ...
    const existingRows = await query(
      "SELECT id FROM orcamentos_itens WHERE orcamento_numero = ? AND id = ?",
      [numero, id]
    ) as any[]
    ...
    // Deletar item
    await query("DELETE FROM orcamentos_itens WHERE orcamento_numero = ? AND id = ?", [numero, id])
